...
public class ArrivalBoardListFragment extends ListFragment {
    ...
    public ArrayList<ArrivalBoard> getArrivalBoardsForSelectedStation() {
        // get list of arrival board from the iterator
        // you cannot just call station.iterator() method because iterator is not a list (sometimes)
        Station station = StationManager.getInstance().getSelected();
        ArrayList<ArrivalBoard> ab = new ArrayList<ArrivalBoard>();
        for (ArrivalBoard a : station){
            ab.add(a);
        }
        return ab;
    }
